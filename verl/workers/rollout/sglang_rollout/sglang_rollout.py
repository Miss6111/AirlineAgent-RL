    ...
    async def _async_rollout_a_request(
        ...
        **kwargs,
        ...
        reward_part_scores = {
            ...
            "executed_mutating_tool_call_count": [],
        ...
        while current_turns < self.config.multi_turn.max_assistant_turns:
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                ...
                for key in reward_part_scores:
                    if key in metrics:
                        reward_part_scores[key].append(metrics[key])
        ...
        # Forced-done reward for TRICK 3 (PRM-Lite): if the rollout ended by
        # max length / max turns / tool-call cutoff before a normal ###STOP###
        # user-simulator turn, still score the process trace and apply a small
        # truncation penalty in the interaction.
        if (
            _req.interaction_kwargs
            and self.interaction_map
            and not reward_part_scores["final_reward"]
        ):
            interaction_name = _req.interaction_kwargs.get("name")
            if interaction_name is None:
                interaction_name = next(iter(self.interaction_map)) if len(self.interaction_map) == 1 else "gsm8k"
            interaction = self.interaction_map.get(interaction_name)
            if interaction is not None and hasattr(interaction, "finalize_forced_reward"):
                messages: list[dict[str, Any]] = []
                for x in _req.messages:
                    msg = {"role": x.role, "content": x.content}
                    if x.tool_calls is not None:
                        msg["tool_calls"] = [tc.model_dump() for tc in x.tool_calls]
                    messages.append(msg)
                reason = finish_reason_type.value if finish_reason_type is not None else "forced_done"
                forced_reward, forced_metrics = await interaction.finalize_forced_reward(
                    _req.request_id,
                    messages,
                    reason=reason,
                    **_req.interaction_kwargs,
                )
                user_turn_rewards.append(forced_reward)
                for key in reward_part_scores:
                    if key in forced_metrics:
                        reward_part_scores[key].append(forced_metrics[key])
        ...
        all_rewards = {**tool_reward_scores, **{"user_turn_rewards": user_turn_rewards}, **reward_part_scores}

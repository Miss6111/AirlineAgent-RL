...
_DEBUG_GRPO_ADV_PRINT_COUNT = 0
_DEBUG_GRPO_LOSS_PRINT_COUNT = 0


def _debug_grpo_enabled() -> bool:
    return os.getenv("TAU_RETAIL_DEBUG_GRPO", "1") != "0"


def _debug_grpo_max_prints() -> int:
    return int(os.getenv("TAU_RETAIL_DEBUG_GRPO_MAX_PRINTS", "20"))
...
def compute_grpo_outcome_advantage(
    ...
    Returns:
        ...
        Returns: `(torch.Tensor)`
            shape is (bs, response_length)
    """
    global _DEBUG_GRPO_ADV_PRINT_COUNT
    ...
    if _debug_grpo_enabled() and _DEBUG_GRPO_ADV_PRINT_COUNT < _debug_grpo_max_prints():
        rollout_n = int(os.getenv("ROLLOUT_N", "0") or 0)
        if rollout_n <= 0:
            rollout_n = max((len(v) for v in id2score.values()), default=0)
        print("[TAU_DEBUG_GRPO] rollout_n =", rollout_n, flush=True)
        if rollout_n > 0 and raw_scores.numel() % rollout_n == 0:
            scores_grouped = raw_scores.view(-1, rollout_n)
            group_sums = scores_grouped.sum(dim=1)
            print("[TAU_DEBUG_GRPO] scores_grouped[:20] =", scores_grouped[:20], flush=True)
            print("[TAU_DEBUG_GRPO] num_all_zero =", (group_sums == 0).sum().item(), flush=True)
            print("[TAU_DEBUG_GRPO] num_all_one =", (group_sums == rollout_n).sum().item(), flush=True)
            print(
                "[TAU_DEBUG_GRPO] num_mixed =",
                ((group_sums > 0) & (group_sums < rollout_n)).sum().item(),
                flush=True,
            )
        else:
            print(
                "[TAU_DEBUG_GRPO] skip scores_grouped: raw_scores.numel =",
                raw_scores.numel(),
                "rollout_n =",
                rollout_n,
                flush=True,
        ...
        advantages = scores
        print("[TAU_DEBUG_GRPO] advantages mean:", advantages.mean().item(), flush=True)
        print("[TAU_DEBUG_GRPO] advantages std:", advantages.std().item(), flush=True)
        print("[TAU_DEBUG_GRPO] advantages min:", advantages.min().item(), flush=True)
        print("[TAU_DEBUG_GRPO] advantages max:", advantages.max().item(), flush=True)
        print(
            "[TAU_DEBUG_GRPO] nonzero advantages:",
            (advantages.abs() > 1e-8).sum().item(),
            "/",
            advantages.numel(),
            flush=True,
        )
        _DEBUG_GRPO_ADV_PRINT_COUNT += 1

    return scores, scores


# LAS only: Length-aware Advantage Scaling without turn discounting.
# This is used for ablation when TAU_AIRLINE_ENABLE_TRICK1=0 and
# TAU_AIRLINE_ENABLE_TRICK2=1. It keeps vanilla GRPO group centering, then
# divides token-level advantages by sqrt(response_length).
@register_adv_est("grpo_length_aware")
def compute_grpo_length_aware_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    ...
    with torch.no_grad():
        ...
        active_lengths = response_mask.sum(dim=1, keepdim=True).clamp(min=1).to(torch.float32)
        length_norm = torch.sqrt(active_lengths)
        scores = scores.unsqueeze(-1) * response_mask / length_norm

    return scores, scores


# TRICK 1: Turn-Discounted Advantage.
# Earlier response tokens receive a larger normalized weight, which protects
# early multi-turn planning from being washed out by later tool chatter.
...
def compute_grpo_turn_discounted_outcome_advantage(
    ...
    with torch.no_grad():
        ...
        weights_stable = torch.exp(log_weights - log_weights_max)
        weight_sum = (weights_stable * response_mask.to(torch.float64)).sum(dim=1, keepdim=True).clamp(min=epsilon)
        weights = (weights_stable * active_lengths / weight_sum).to(torch.float32)
...
# LAS: Length-aware Advantage Scaling.
# It keeps Trick 1's turn weighting, then divides by sqrt(L) rather than letting
# token-mean aggregation impose a near-linear penalty on longer reasoning.
@register_adv_est("grpo_las")
def compute_grpo_las_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
...
def compute_policy_loss_vanilla(
    ...
    global _DEBUG_GRPO_LOSS_PRINT_COUNT
    if _debug_grpo_enabled() and _DEBUG_GRPO_LOSS_PRINT_COUNT < _debug_grpo_max_prints():
        mask = response_mask.bool()
        print("[TAU_DEBUG_LOSS] response_mask.sum():", response_mask.sum().item(), flush=True)
        if mask.any():
            print("[TAU_DEBUG_LOSS] policy_log_probs.abs().mean():", log_prob[mask].abs().mean().item(), flush=True)
            print("[TAU_DEBUG_LOSS] old_log_probs.abs().mean():", old_log_prob[mask].abs().mean().item(), flush=True)
            print("[TAU_DEBUG_LOSS] ratio.mean():", ratio[mask].mean().item(), flush=True)
            print("[TAU_DEBUG_LOSS] ratio.min():", ratio[mask].min().item(), flush=True)
            print("[TAU_DEBUG_LOSS] ratio.max():", ratio[mask].max().item(), flush=True)
        else:
            print("[TAU_DEBUG_LOSS] empty response mask", flush=True)
        print("[TAU_DEBUG_LOSS] pg_loss:", pg_loss.detach().item(), flush=True)
        _DEBUG_GRPO_LOSS_PRINT_COUNT += 1
